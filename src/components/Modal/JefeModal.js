...
const ClientModal = ({ dataClient, open, setOpen, cancelButtonRef }) => {
  ...
  const handleClickSearch = async (e) => {
    ...
    try {
      const { status, data } = await fetchClientesDocumento(searchDocument,'JEFE');

      if (status == 200) {
        toastMessage("Jefe encontrado", 1);
        dataClient.id_jefe = data.id_persona;
        dataClient.jefe = `Nombre: ${data.nombre}`;
        setClient({
          id: data.id_persona,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (status == 201) {
        dataClient.id_jefe = data.id_persona;
        dataClient.jefe = `Nombre: ${client.names}`;
