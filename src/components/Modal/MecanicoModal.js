...
const ClientModal = ({ dataClient, open, setOpen, cancelButtonRef }) => {
  ...
  const handleClickSearch = async (e) => {
    ...
    try {
      const { status, data } = await fetchClientesDocumento(searchDocument,'MECANICO');

      if (status == 200) {
        toastMessage("Mecanico encontrado", 1);
        dataClient.id_mecanico = data.id_persona;
        dataClient.mecanico = `Nombre: ${data.nombre}`;
        setClient({
          id: data.id_persona,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (status == 201) {
        dataClient.id_mecanico = data.id_persona;
        dataClient.mecanico = `Nombre: ${client.names}`;
