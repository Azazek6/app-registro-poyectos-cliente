...
const ClientModal = ({ dataClient, open, setOpen, cancelButtonRef }) => {
  ...
  const handleClickSearch = async (e) => {
    ...
    try {
      const { status, data } = await fetchClientesDocumento(searchDocument,'RESIDENTE');

      if (status == 200) {
        toastMessage("Residente encontrado", 1);
        dataClient.id_residente = data.id_persona;
        dataClient.residente = `Nombre: ${data.nombre}`;
        setClient({
          id: data.id_persona,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (status == 201) {
        dataClient.id_residente = data.id_persona;
        dataClient.residente = `Nombre: ${client.names}`;
